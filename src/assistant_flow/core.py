...
class AssistantAPI:
    ...
    def run(self, question):
        ...
        try:
            ...
            logging.error(f"Failed to submit the message: {e}")
            self.queue.end()
            return    
        ...
        if run.status == "completed":
            ...
            logging.info(f"Run status: {run.status} error: {run.last_error}")
            self.queue.send(f"Run failed with status: {run.status}  error: {run.last_error}")
